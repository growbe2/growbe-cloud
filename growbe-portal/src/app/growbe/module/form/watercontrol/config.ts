import { AutoFormData, FormObject, FormProperty, UnionProperty } from "@berlingoqc/ngx-autoform";
import { of } from "rxjs";
import { GrowbeModuleAPI } from "src/app/growbe/api/growbe-module";


export const getModuleWaterControlConfig: (moduleId: string, config: any, moduleDef: any, growbeModuleAPI: GrowbeModuleAPI) => AutoFormData = (
  moduleId: string, config: any, moduleDef: any, growbeModuleAPI: GrowbeModuleAPI
) => ({
  type: 'simple',
  items: [
    {
      ...
      properties: [
        ...(["p0", "p1", "p2"].map((itext) => {
          return {
            type: 'object',
            templates: {
              'header': (moduleDef.properties[itext].displayName) ? moduleDef.properties[itext].displayName: itext,
            },
            name: itext,
            properties: [
              {
